...
int checkKeyPairUniqueness(struct Drugstore drugstore, int productId)
{
...
void info()
{
    FILE* indexTable = fopen("drugstore.ind", "rb");
    ...
    {
        if (getDrugstore(&client, i, dummy) && client.id > 0)
        {
            clientCount++;
            orderCount += client.drug_count;

            printf("Client #%d has %d order(s)\n", i, client.drug_count);
        }
    }
