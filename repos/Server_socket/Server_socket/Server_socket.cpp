...
string convertFiletoString(string& nameOfFile) {
    ...
    char sym;
    char lastSym = '\0';
    while (!file.eof()) // пока файл не порожній, зчитуємо з файлу по одному символу і вводимо значення змінної
    {
        file >> sym;
        if (sym == '\n') {
            msg1.push_back(' ');
...
int main() {
    ...
    ofstream log("ServerLog.txt");//потрібно створити "ServerLog.txt"
    if (log.is_open() == true) {
    ...
    while (1) {
        ...
        if (msg == "Who") {
            ...
            if (msg == msgforcomp) {
                msg = "Your message is quaziidentifical with \"1S.txt\"";
                Send(ListenSocket, msg, log);
            }
            else if (msg == msgforcomp2) {
                msg = "Your message is quaziidentifical with \"2S.txt\"";
                Send(ListenSocket, msg, log);
            }
            else {
                msg = "Your message isn`t quaziidentifical";
                Send(ListenSocket, msg, log);
            }
        }
    }
